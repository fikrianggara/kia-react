...
export const MaternalSelection = ({ selection, data, onSelect }) => {
  return (
      ...
      <div className='h-8'>
        <Menu.Button className="w-full justify-center rounded-md h-full bg-black bg-opacity-20 px-4 text-sm font-medium text-white hover:bg-opacity-30 focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75">
          <div className='flex align-middle '>
            <span className='flex flex-col mb-0'>Kelahiran {selection?.kelahiran_ke}</span>
          ...
          <div className="px-1 py-1 ">
            {data.map(item => (
              <Menu.Item key={item.kelahiran_id}>
                {({ active }) => (
                  <button onClick={() => onSelect(item)} className={`${active ? 'bg-sky-600 text-white' : 'text-gray-900'} group flex w-full items-center rounded-md px-2 py-2 text-sm`}>
